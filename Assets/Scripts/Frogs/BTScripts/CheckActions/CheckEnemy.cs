...
public class CheckEnemy : ActionNode
{
    ...
    protected override void OnStart() {
        ...
        {
            bool isLOS = true; // Assume line of sight until proven otherwise

            Ray ray = new Ray(context.transform.position, collider.transform.position - context.transform.position);
            float distance = Vector3.Distance(context.transform.position, collider.transform.position);
            RaycastHit[] hits = Physics.RaycastAll(ray, distance, ~LayerMask.GetMask("Frog"));

            foreach (RaycastHit hit in hits)
            {
                if (hit.collider.gameObject.layer == LayerMask.GetMask("BlockLOS"))
                {
                    // Found an obstruction, line of sight is blocked
                    isLOS = false;
                    break;
                }
            }
            ...
            if (isLOS)
            {
                blackboard.collidersInLOS.Add(collider);
            }
        }
